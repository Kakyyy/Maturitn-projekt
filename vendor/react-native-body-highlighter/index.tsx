...
const Body = ({
  ...
  const handlePress = useCallback(
    (bodyPart: ExtendedBodyPart, pressedSide?: "left" | "right") => {
      ...
      if (
        last &&
        last.slug === bodyPart.slug &&
        last.side === pressedSide &&
        now - last.time < 250
      ) {
        return;
      }
        ...
        {mergedBodyParts(bodyToRender).map((bodyPart: ExtendedBodyPart) => {
          const commonPaths = (bodyPart.path?.common || []).map((path) => {
            ...
            return (
              <Path
                key={path}
                onPressIn={() => handlePress(bodyPart)}
          ...
          const leftPaths = (bodyPart.path?.left || []).map((path) => {
            ...
            return (
              <Path
                key={path}
                onPressIn={() => handlePress(bodyPart, "left")}
                onPress={() => handlePress(bodyPart, "left")}
                id={bodyPart.slug}
          ...
          const rightPaths = (bodyPart.path?.right || []).map((path) => {
            ...
            return (
              <Path
                key={path}
                onPressIn={() => handlePress(bodyPart, "right")}
                onPress={() => handlePress(bodyPart, "right")}
                id={bodyPart.slug}
